...
A one-paragraph hook or angle that differentiates this article from competitors.`;
    ...
    // Step 2b: Gather verified sources via web search.
    // The writing step is constrained to ONLY cite sources from this list.
    // Anything the writer cites outside this list gets stripped after generation.
    ...
    try {
      verifiedSources = await gatherVerifiedSources(
        client,
        article.topic,
      ...
      await logStep(
        ...
        verifiedSources.length > 0
          ? `Found ${verifiedSources.length} verified source(s)`
          : "No sources found — article will be written without citations",
      );
    } catch (err) {
      logger.warn({ articleId, err }, "Source gathering failed; article will proceed without verified sources");
      await logStep(articleId, "source_gathering", "failed", "Source gathering errored — article will be written without citations");
    }

    // Build a compact text representation of the verified sources for the
    // writing prompt. The writer is told this is the ONLY allowed citation
    // pool. Anything cited outside this list will be stripped.
    const sourcesBlock = verifiedSources.length > 0
      ? verifiedSources
          .map(
            (s, i) =>
              `[S${i + 1}] ${s.title}\n     URL: ${s.url}${s.publishedDate ? ` (${s.publishedDate})` : ""}\n     Summary: ${s.snippet.slice(0, 250)}`,
          )
          .join("\n\n")
      : "";

    // Step 3: Write article (with word-count retry loop)
    await updateArticleStatus(articleId, "writing");

    const targetWords = article.wordCountTarget;
    const wordCountTolerance = 200;
    const minWords = targetWords - wordCountTolerance;
    const maxWords = targetWords + wordCountTolerance;

    const buildWritingPrompt = (driftHint?: { actualWords: number; previous: string }) => {
      const driftSection = driftHint
        ? `\n\nPREVIOUS ATTEMPT WORD COUNT: ${driftHint.actualWords} words (target was ${targetWords} ±${wordCountTolerance}, so ${minWords}-${maxWords}).
${driftHint.actualWords < minWords
  ? `That draft was TOO SHORT. Expand sections with more concrete detail, more specific examples, more data points. Do not pad with filler — add real substance.`
  : `That draft was TOO LONG. Tighten sections by cutting filler, redundant transitions, and over-explanation. Preserve facts and structure.`}
`
        : "";
...
- Target word count: ${targetWords} words. STRICT acceptable range: ${minWords}-${maxWords} words. Drafts outside that range will be regenerated.
...
${article.tone ? `Write in the tone described above.` : "Write in a formal, expert voice."} Prefer direct, concrete statements with named sources and specific numbers. Start the article with the H1 line — no preamble, no commentary.${driftSection}`;
    };

    let articleDraft = "";
    let wordCountAttempts = 0;
    const MAX_WORD_COUNT_ATTEMPTS = 3; // 1 initial + 2 retries
    let lastDraft = "";
    let lastWords = 0;

    while (wordCountAttempts < MAX_WORD_COUNT_ATTEMPTS) {
      wordCountAttempts++;
      const stepLabel = wordCountAttempts === 1 ? "writing" : `writing_retry_${wordCountAttempts - 1}`;
      await logStep(
        articleId,
        stepLabel,
        "running",
        wordCountAttempts === 1
          ? `Generating article draft (target ${targetWords} ±${wordCountTolerance} words)`
          : `Word count was ${lastWords} (need ${minWords}-${maxWords}); regenerating`,
      );

      const driftHint = wordCountAttempts > 1 ? { actualWords: lastWords, previous: lastDraft } : undefined;
      articleDraft = await callClaude(client, buildWritingPrompt(driftHint), 8192, {
        temperature: 0.85,
      ...
      await logStep(articleId, stepLabel, "completed", `Draft generated (${draftWords} words)`);

      if (draftWords >= minWords && draftWords <= maxWords) {
        break; // in band, accept
      }

      lastDraft = articleDraft;
      lastWords = draftWords;
    }

    const finalDraftWords = countWords(articleDraft);
    const wordCountOutOfBand = finalDraftWords < minWords || finalDraftWords > maxWords;
    if (wordCountOutOfBand) {
      await logStep(
        articleId,
        "word_count_check",
        "failed",
        `Final draft is ${finalDraftWords} words, outside ${minWords}-${maxWords} after ${MAX_WORD_COUNT_ATTEMPTS} attempts. Article will ship with out-of-band flag.`,
      );
    } else {
      await logStep(
        articleId,
        "word_count_check",
        "completed",
        `Final draft ${finalDraftWords} words within target ${minWords}-${maxWords}`,
      );
    }

    // Step 3b: Citation verification.
    // Extract every citation from the draft, check against the verified source
    // list, and strip anything that doesn't match. This is mechanical (regex-
    // based extraction + targeted removal) rather than a model rewrite, so it's
    // predictable and cheap.
    await logStep(articleId, "citation_verification", "running", "Extracting and verifying citations");
    ...
    await logStep(
      ...
      "completed",
      `Found ${extracted.length} citation(s); ${verifiedCitationCount} verified, ${citationStripped} unverified citation(s) stripped`,
    );

    // Step 4: ZeroGPT humanization + scoring
    await updateArticleStatus(articleId, "humanizing");
    let finalArticle = articleAfterCitationCheck;
    ...
    if (isZeroGptConfigured()) {
      ...
      try {
        const humanized = await humanizeText(articleAfterCitationCheck, article.tone);
    ...
    // Heading keyword check with one auto-fix attempt.
    ...
    let headingCheck = checkHeadingKeywords(finalArticle, article.primaryKeyword, article.secondaryKeywords);
    ...
    if (headingViolationsInitial.length > 0) {
      await logStep(
        ...
        "heading_check",
        "running",
        `Heading violations detected (${headingViolationsInitial.join("; ")}). Attempting one model rewrite.`,
      );
...
${finalArticle}`;
        ...
        if (fixed.trim().length > 0) {
          ...
          if (recheckViolations.length < headingViolationsInitial.length) {
            // The fix improved things — accept it. Even if not perfect, fewer
            // violations is better than the original.
            finalArticle = fixed;
            ...
            await logStep(
              ...
              recheckViolations.length === 0
                ...
                : `Heading rewrite reduced violations to: ${recheckViolations.join("; ")}. Article shipping with remaining issues flagged.`,
            );
          } else {
            await logStep(
              articleId,
              "heading_check",
              "failed",
              `Heading rewrite did not improve violations. Article shipping with: ${headingViolationsInitial.join("; ")}`,
            );
          }
        } else {
          await logStep(articleId, "heading_check", "failed", "Heading rewrite returned empty content. Article shipping with original headings.");
        }
      } catch (err) {
        const msg = err instanceof Error ? err.message : String(err);
        logger.warn({ articleId, err }, "Heading rewrite attempt errored");
        await logStep(articleId, "heading_check", "failed", `Heading rewrite errored: ${msg}. Article shipping with original headings.`);
      }
    } else {
      await logStep(
        articleId,
    ...
    const headingViolationsFinal = summarizeHeadingViolations(headingCheck);
